// Script to replace conflicted wiki-updater.js
const fs = require('fs');
const path = require('path');

// Path to the conflicted file
const basePath = process.env.APPDATA ? 
  path.join(process.env.APPDATA, 'Claude', 'dokkan-mcp', 'lib') : 
  path.join(require('os').homedir(), '.claude', 'dokkan-mcp', 'lib');

// Get the correct path based on your directory structure
const conflictedFilePath = path.join(basePath, 'wiki-updater.js');
const tempFilePath = path.join(basePath, 'wiki-updater-new.js');

// The complete content you want to save
const newContent = `const fsSync = require('fs');
...
try {
  ...
  console.log(`Successfully created temporary file at ${tempFilePath}`);
  
  // Check if the original file exists before trying to delete it
  if (fs.existsSync(conflictedFilePath)) {
    // Now try to delete the original file
    fs.unlinkSync(conflictedFilePath);
    console.log(`Successfully deleted conflicted file at ${conflictedFilePath}`);
  }
  
  // Now rename the temporary file to the original filename
  fs.renameSync(tempFilePath, conflictedFilePath);
  console.log(`Successfully renamed temporary file to ${conflictedFilePath}`);
  
  console.log('MERGE CONFLICT RESOLVED!');
} catch (error) {
  console.error('Error during file operations:', error);
  console.log('\nMANUAL STEPS:');
  console.log('1. Close VS Code without saving');
  console.log('2. Delete the file: ' + conflictedFilePath);
  console.log('3. Create a new file with the same name');
  console.log('4. Copy-paste the content from paste.txt into the new file');
}